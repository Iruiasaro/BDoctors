...
<main class="d-flex p-4" :class="isSearched ? 'no-height' : ''">
    <div class="container">
        <form class="shadow-lg p-3 mb-5 bg-body rounded" style="max-width:700px;background-color:#168d7d;" action="" @submit.prevent="onSubmit">
        ...
        <div class="m-2">
            <form action="">
                ...
                <div v-for="index in 5" class="form-check form-check-inline" v-on:change="onChangeStar()">
                    ...
                    <label class="form-check-label" :for="index"><span v-for="item in index"><i class="fa fa-star third-color" aria-hidden="true"></i> </span> </label>
                </div>
            </form>
        </div>
        <div class="m-2">
            <button class="btn btn-outline-primary p-1" v-on:click="sortByReviewsNumber()"> <i class="fa m-2" :class="!sortValue ? 'fa-sort-amount-asc':'fa-sort-amount-desc'" aria-hidden="true"></i>Filtra per
                    ...
                    <p class="card-text">
                        <div>
                            <ul class="list-unstyled ">
                                <li style="padding-left:2px;margin-top:5px;border:1px solid black;border-radius:15px" v-for="specialization in doctor.specializationsArray"><i style="padding:3px; ">
                                        @{{ specialization . specs_type }} </i></li>
                            </ul>
                        </div>
                        <h6> <i class="fa fa-map-marker" aria-hidden="true"></i> @{{ doctor . cityName }} </h6>
                        <h6> <i class="fa fa-star-half-o" aria-hidden="true"></i> Recensioni:
                            (@{{ doctor . reviews . length }})
                        </h6>
                    </p>
                    <div class="mt-2 mb-2"> <i v-for="index in Math.round(doctor.voteAverage)" class="fa fa-star third-color"> </i></div>
                    <a :href='show(doctor.id)' class="btn btn-primary mt-auto" style="width: 100%">Visualizza
                        Profilo</a>
                </div>
            </div>
        </div>
    ...
    <div class="justify-content-between ">
        <div>
            ...
            <div class="suggested-doctors d-flex flex-wrap">
                <div v-for="doctor in sponsorizedUsers" class="card" style="">
                    <div class="card-img-top overflow-hidden" style="min-height:200px">
                        <img v-if="!doctor.image" src="{{ asset('imgs/avatar.png') }}" alt="" style="width:100%">
                        <img v-else :src="doctor.image.includes('random')?doctor.image:'storage/' + doctor.image" alt="Card image cap" style="width:100%">
                    </div>
                    <div class="card-body d-flex flex-column">
                        ...
                        <p class="card-text">
                            <div>
                                <ul class="list-unstyled">
                                    <li style="padding-left:4px;margin-top:5px;border:1px solid black;border-radius:15px" v-for="specialization in doctor.specializationsArray"><i>
                                            @{{ specialization.specs_type }} </i></li>
                                </ul>
                            </div>
                            <h6> <i class="fa fa-map-marker" aria-hidden="true"></i> @{{ doctor.cityName }} </h6>
